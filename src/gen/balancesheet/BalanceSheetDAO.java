...
import gen.profitandloss.ProfitAndLossDAO;
...
public class BalanceSheetDAO {
    ...
    public static List<TrialBalanceDTO> getGroupTransactions(List<String> groups, String fromDateStr, String toDateStr, int check, int Call_For_Profit_Or_Balance_transaction) throws SQLException, IOException {
	Double finalTotal = 0D;

	List<TrialBalanceDTO> groupDTOList = new ArrayList<TrialBalanceDTO>();
	Map<String, List<String>> mapMainGroupIdWithSubGroupList = getGroupsHierarchy(Call_For_Profit_Or_Balance_transaction);

	for (Map.Entry<String, List<String>> e : mapMainGroupIdWithSubGroupList.entrySet()) {
	    TrialBalanceDTO trialBalanceDTO = getTransaction(e.getValue(), fromDateStr, toDateStr, check, Call_For_Profit_Or_Balance_transaction);
	...
	try {
	    ...
	    Logger.getLogger(BalanceSheetDAO.class.getName()).log(Level.SEVERE, null, ex);
	    throw ex;
	}
	return mapToReturn;
    }
    ...
    public static Double getProfitAndLossOpeningBal(String date1, String date2, int check) throws SQLException {
	try {
	    System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" + call_For_Opening);
	    ...
	    String query = "";
	    if (check == 5) {
		System.out.println("Check VAlue in the opening_Balance" + check + "Call_For_Opening         " + call_For_Opening);
//                if (call_For_Opening != 1) {
//                    query = "Select * from tblstockitem where si_applicable_From_date <= '" + date2 + "'";
//                } else if (call_For_Opening == 1) {
		query = "Select * from tblstockitem where applicable_From_date < '" + date1 + "'";
//                }
		System.out.println("Query in the opening_Balance" + query);
	    } else if (check == 10) {
		if (call_For_Opening != 1) {
		    //  query = "Select * from tblstockitem where si_applicable_From_date <= '" + date1 + "'";
	    ...
	    stockItemSaleAmount = getvalue(Constants.SALE_TYPE_INDEX.toString(), date1, date2, check);

	    // calculate opening balnce of StockItem present in Purchse transaction FromDate to ToDate
	    stockItemPurchaseAmount = getvalue(Constants.PURCHASE_TYPE_INDEX.toString(), date1, date2, check);
	    ...
	    System.out.println("Profit opening Balance in Calculate openining Balance" + profitAndLossOpenBal);
    ...
    public static Double getProfitAndLossClosingBal(String date1, String date2, int check) throws SQLException {
	...
	call_For_Opening = 5;
	opening_Balance = getProfitAndLossOpeningBal(date1, date2, check);
	call_For_Opening = 1;

	// set call_for_closing = 5 so that we get transaction of SAle and purchase between FromDate to ToDate
	call_For_Closing = 5;
//
	System.out.println("SALEEEEEEEEEEEEEEE 000000  From Closing" + getvalue(Constants.SALE_TYPE_INDEX.toString(), date1, date2, check));
	System.out.println("PURCHASEEEEEEEEEEEE ))))))))0 From Closing" + getvalue(Constants.PURCHASE_TYPE_INDEX.toString(), date1, date2, check));
	System.out.println("CXALLLLLLL FROM     total closing >>>>>>>>>>>>>>>>>>>" + opening_Balance);

	// calculate total Opening Balance 
	total_Closing_Balance = opening_Balance + getvalue(Constants.PURCHASE_TYPE_INDEX.toString(), date1, date2, check) - getvalue(Constants.SALE_TYPE_INDEX.toString(), date1, date2, check);
	System.out.println("total Closing......... in getProfitAndLossClosingBal" + total_Closing_Balance);
    ...
    public static Double getvalue(String trans_type, String date1, String date2, int check) {
...
//                }
    ...
    public static Map<String, List<String>> getGroupsHierarchy(int Call_For_Profit_Or_Balance_transaction) {
	...
	try {
	    ...
	    String query1 = "";
	    if (Call_For_Profit_Or_Balance_transaction == 5) {
		query1 = "select group_id,group_name,group_under from tblgroup where group_name in " + "('" + Constants.SALES_ACCOUNT + "','" + Constants.PURCHASE_ACCOUNT + "','" + Constants.INCOME_DIRECT + "','" + Constants.INCOME_INDIRECT + "','" + Constants.EXPENSE_DIRECT + "','" + Constants.EXPENSE_INDIRECT + "')";
	    } else if (Call_For_Profit_Or_Balance_transaction == 10) {
		query1 = "select group_id,group_name,group_under from tblgroup where group_name in " + "('" + Constants.CURRENT_ASSETS + "','" + Constants.CURRENT_LIABILITY + "','" + Constants.LOAN_LIABILITY + "','" + Constants.CURRENT_LIABILITY + "','" + Constants.SUSPENSE_ACCOUNT + "')";
	    }
	    PreparedStatement ps1 = conn.prepareStatement(query1);
	    ResultSet rs1 = ps1.executeQuery();
	    Integer groupSize = 0;
		...
		do {
			...
			if (cash_ids.get(index).equalsIgnoreCase(e2.getValue())) {
			    cash_ids.add(e2.getKey());
                            System.out.println("Group Id --- "+e2.getKey());
			}
		    }
		    ...
		    index++;
                ...
                System.out.println("Cash ID ------------ "+cash_ids);
                System.out.println("Call_For_Profit_Or_Balance_transaction ------------ "+Call_For_Profit_Or_Balance_transaction);
    ...
    public static TrialBalanceDTO getTransaction(List<String> GroupNames, String fromDateStr, String toDateStr, int check, int Call_For_Profit_Or_Balance_transaction) throws SQLException {
	...
	System.out.println("Groups for Balance sheet..................." + str1);
	double debit = 00f;
	double credit = 00f;
	int i = 0;
	int flag = 0;
	...
	String query = "";
	if (check == 5 && Call_For_Profit_Or_Balance_transaction == 5) {
	    System.out.println("Value of Check 55555........" + check + "  value of " + Call_For_Profit_Or_Balance_transaction);
	    query = "Select * from tblledger where ledger_under in (" + str1 + ") and applicable_From_date < '" + fromDateStr + "'";
	} else if (check == 10 && Call_For_Profit_Or_Balance_transaction == 5) {
	    System.out.println("Value of Check 55555........" + check + "  value of " + Call_For_Profit_Or_Balance_transaction);
	    query = "Select * from tblledger where ledger_under in (" + str1 + ") and applicable_From_date between '" + fromDateStr + "' and '" + toDateStr + "'";
	} else if (check == 10 && Call_For_Profit_Or_Balance_transaction == 10) {
	    System.out.println("Value of Check 55555........" + check + "  value of " + Call_For_Profit_Or_Balance_transaction);
	    query = "Select * from tblledger where ledger_under in (" + str1 + ") and applicable_From_date <= '" + toDateStr + "'";
	}
	System.out.println("Query for the Ledger in transaction_--------------------------" + query);
	PreparedStatement prmt = conn.prepareStatement(query);
	ResultSet rs = prmt.executeQuery();
	while (rs.next()) {
	...
	System.out.println("Ledger Credit and Debit in getTransaction" + creditLedger_Balance + "    " + debitLedger_Balance);
	...
	if (check == 5 && Call_For_Profit_Or_Balance_transaction == 5) {
//            System.out.println("Value of Check 55555........" + check + "  value of " + Call_For_Profit_Or_Balance_transaction);
	    query2 = "select sum(trans_amt) as totalDeb, trans_type from tbltransactionledger ";
	    whereCondition = "";
	    whereCondition1 = "where trans_id in";
	    subQuery1 = "(select trans_id from tbltransactionmain ";
	    dateQuery = "where trans_date < '" + fromDateStr + "')";
	    whereCondition2 = "and trans_ledgerid in";
	    subQuery2 = "(select ledger_id from tblledger ";
	} else if (check == 10 && Call_For_Profit_Or_Balance_transaction == 5) {
//            System.out.println("Value of Check 100000........" + check + "  value of " + Call_For_Profit_Or_Balance_transaction);
	    query2 = "select sum(trans_amt) as totalDeb, trans_type from tbltransactionledger ";
	    whereCondition = "";
	    whereCondition1 = "where trans_id in";
	    subQuery1 = "(select trans_id from tbltransactionmain ";
	    // dateQuery = "where trans_date>='" + fromDateStr + "' and trans_date<='" + toDateStr + "')";
	    dateQuery = "where trans_date Between '" + fromDateStr + "' and '" + toDateStr + "')";
	    whereCondition2 = "and trans_ledgerid in";
	    subQuery2 = "(select ledger_id from tblledger ";
	} else if (check == 10 && Call_For_Profit_Or_Balance_transaction == 10) {
//            System.out.println("Value of Check 5 100........" + check + "  value of " + Call_For_Profit_Or_Balance_transaction);
	    query2 = "select sum(trans_amt) as totalDeb, trans_type from tbltransactionledger ";
	    whereCondition = "";
	    whereCondition1 = "where trans_id in";
	    subQuery1 = "(select trans_id from tbltransactionmain ";
	    dateQuery = "where trans_date <= '" + toDateStr + "')";
	    whereCondition2 = "and trans_ledgerid in";
	    subQuery2 = "(select ledger_id from tblledger ";
	}
	...
	System.out.println("QUERY2 for credit and Dxebit of transaction......................." + query2);
	PreparedStatement ps2 = conn.prepareStatement(query2);
	ResultSet rs2 = ps2.executeQuery();
	while (rs2.next()) {
