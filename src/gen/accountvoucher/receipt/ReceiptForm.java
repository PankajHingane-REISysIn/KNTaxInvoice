 ...
 * @author pc5
 */
public class ReceiptForm extends javax.swing.JInternalFrame implements java.awt.event.ActionListener {
    ...
    List<ReceiptDTO> receiptDTOTransactionList;
    ...
    //variable for focus
    private int currentFocusValue = 1;
    //time stamp
    ...
    public ReceiptForm(String s, Dimension d) {
        try {
            initComponents();
            ...
            JOptionPane.showMessageDialog(ReceiptForm.this, ex.getMessage());
            ex.printStackTrace();
        }
    }

    public ReceiptForm(ReceiptDTO receiptDTO, Boolean isEdit) {
        try {
            initComponents();
            this.isEdit = isEdit;
    ...
    private void initComponents() {
        ...
        setFrameIcon(new ImageIcon(ReceiptForm.class.getResource("/images/Kasturi-logo-1.png")));
        setTitle("Receipt Transaction Form");
        setBounds(100, 100, 1332, 674);

        jMainPanel = new JPanel();
        jMainPanel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Receipt Form", TitledBorder.LEADING, TitledBorder.TOP, null, null));
        ...
        String col1[] = {Label.RECEIPT_NO, Label.DATE_TRANSACTION, Label.SUPPLIER_BUYER_NAME};
        String data1[][] = {{"", "", ""}};
        ...
        panel_2.setLayout(new MigLayout(
                "",
                "[0px:200px:200px,grow,shrink 50,fill][0px:100px:100px,grow,shrink 50,fill][0px:100px:100px,grow,shrink 50,fill][0px:165px:165px,grow,shrink 50,fill][0px:50px:50px,grow,shrink 50,fill][grow]",
...
//                "[0px:25px:25px,grow,shrink 50,fill][0px:25px:25px,grow,shrink 50,fill][0px:25px:25px,grow,shrink 50,fill][0px:25px:25px,grow,shrink 50,fill]"));
    ...
    private void jTextFieldReceiptNoFocusLost(java.awt.event.FocusEvent evt) throws Exception {
        if (!isEdit) {
            ...
            if (jTextFieldReceiptNo.getText().trim().equalsIgnoreCase("")) {
                jTextFieldReceiptNo.setText(AccountingVoucherHelper.getNextAccountVoucherNumber(Constants.RECEIPT_TYPE_INDEX) + "");
            }
        }
    }
    ...
    private void jTextFieldSearchKeyReleased(java.awt.event.KeyEvent evt) throws Exception {
        ...
        for (ReceiptDTO receiptDTOEntity : receiptDTOTransactionList) {
    ...
    private void buttonBackActionPerformed(java.awt.event.ActionEvent evt) throws Exception {
        ...
        m.menuselection(4);
        ReceiptForm.this.setClosed(true);
    }
    ...
    private void buttonDeleteActionPerformed(java.awt.event.ActionEvent evt) throws Exception {
        ReceiptDAO.deleteTransaction(Long.parseLong(receiptDTO.getTrans_ID()));
    ...
    private void buttonPrintActionPerformed(java.awt.event.ActionEvent evt) throws Exception {
        if (buttonDelete.isEnabled()) {
            ...
            parameter.put("TitleHeader", "Receipt");
    ...
    private void initVariables() throws Exception {
        initLedger();
        if (!isEdit) {
            receiptDTO.setReceiptNo(AccountingVoucherHelper.getNextAccountVoucherNumber(Constants.RECEIPT_TYPE_INDEX));
        }
...
//        groups.add(Constants.CURRENT_ASSETS);
    ...
    private void bindTOGUI() throws ParseException, SQLException, Exception {
        ...
        Long number = (gen.accountvoucher.chalan.ChalanDAO.getCount_transaction(Constants.RECEIPT_TYPE_INDEX, Constants.DEBIT) / gen.other.AdSuMuDiSettingsOLD.AdSuMuDiConstant.map_checkBoxValue.get("pagination"));
        Long remaining = (gen.accountvoucher.chalan.ChalanDAO.getCount_transaction(Constants.RECEIPT_TYPE_INDEX, Constants.DEBIT) % gen.other.AdSuMuDiSettingsOLD.AdSuMuDiConstant.map_checkBoxValue.get("pagination"));;
        if (remaining == 0L) {
    ...
    private void bindDTOtoGUI() throws Exception {
        tfAccountText.setText(receiptDTO.getAccountName());
        jTextFieldReceiptNo.setText(receiptDTO.getReceiptNo().toString());
        labelFinalAmountTotal.setText(Constants.DECIMAL_FORMAT.format(receiptDTO.getFinalAmount()) + "");
        if (!isEdit) {
            jDateChooser.setDate((java.util.Date) Constants.DATE_FORMATER.parse(receiptDTO.getDate().trim()));
        } else {
            jDateChooser.setDate((java.util.Date) Constants.simpleDateFormatDatabaseWithDash.parse(receiptDTO.getDate().trim()));
        }

        if (receiptDTO.getLedgerTransactionDTOList().isEmpty() || receiptDTO.getLedgerTransactionDTOList() == null) {
    ...
    private void bindDTOtoJtable() throws Exception {
        int index = 1;
        List<LedgerTransactionDTO> stockItemTransactionDTOList = receiptDTO.getLedgerTransactionDTOList();
    ...
    private void bindDTOtoGUIWithoutReceiptNo() throws Exception {
        tfAccountText.setText(receiptDTO.getAccountName());
        jTextFieldNote.setText(receiptDTO.getNote());
        labelFinalAmountTotal.setText(Constants.DECIMAL_FORMAT.format(receiptDTO.getFinalAmount()) + "");
        tfDatePicker.setText(new ReceiptDTO().getDate());
    }

    private void submit() throws SQLException, ParseException, Exception {
        receiptDTO = bindGUITODTO();
        receiptDTO = ImpExpUtil.valiDateAndRepairReceiptDTO(receiptDTO);
        receiptDTO.setAccountName(accountLedgerMap.get(receiptDTO.getAccountName().trim().toLowerCase()));
    ...
    private ReceiptDTO bindGUITODTO() throws Exception {
        ReceiptDTO receiptDTOEntity = new ReceiptDTO();
        receiptDTOEntity.setTrans_ID(receiptDTO.getTrans_ID());

        if (!jTextFieldReceiptNo.getText().trim().equalsIgnoreCase("")) {
            receiptDTOEntity.setReceiptNo(Integer.parseInt(jTextFieldReceiptNo.getText()));
        }
        if (!jDateChooser.getDate().toString().trim().equalsIgnoreCase("")) {
            receiptDTOEntity.setDate(Constants.simpleDateFormatDatabaseWithDash.format(jDateChooser.getDate()).toString().trim());
        }
        if (!tfAccountText.getText().trim().equalsIgnoreCase("")) {
            receiptDTOEntity.setAccountName(tfAccountText.getText().trim());
        }
        if (!jTextFieldCheckNo.getText().trim().equalsIgnoreCase("")) {
            receiptDTOEntity.setCheckNo(jTextFieldCheckNo.getText().trim());
        }
        if (!jTextFieldNote.getText().trim().equalsIgnoreCase("")) {
            receiptDTOEntity.setNote(jTextFieldNote.getText().trim());
        }

        receiptDTOEntity.setLedgerTransactionDTOList(bindtableToDTO());

        if (!labelFinalAmountTotal.getText().trim().equalsIgnoreCase("")) {
            receiptDTOEntity.setFinalAmount(Double.parseDouble(labelFinalAmountTotal.getText().trim()));
        }

        bindtableToDTO();

        return receiptDTOEntity;
    }
    ...
    private void insertReceipt() throws SQLException, ParseException, Exception {
        ...
        ReceiptDAO.insertReceiptVoucher(receiptDTOList);
        AccountingVoucherHelper.updateVoucherNumber(Constants.RECEIPT_TYPE_INDEX, (receiptDTO.getReceiptNo()));
    }

    private void updateReceipt() throws SQLException, ParseException, Exception {
        List<ReceiptDTO> receiptDTOList = new ArrayList<ReceiptDTO>();
        receiptDTOList.add(receiptDTO);
        ReceiptDAO.updateReceiptVoucher(receiptDTOList);
    }
    ...
    public void loadEditForm(String id) throws SQLException, ParseException, Exception {
        ...
        idSet.add(id);
        List<ReceiptDTO> receiptDTOList = ReceiptDAO.getReceipts(idSet,"");
        if (receiptDTOList != null && !receiptDTOList.isEmpty()) {
            receiptDTO = receiptDTOList.get(0);
    ...
    private void bindDTOToTransactionTable(List<ReceiptDTO> receiptDTOTransactionList) throws Exception {
        if (receiptDTOTransactionList != null && !receiptDTOTransactionList.isEmpty()) {
            transactionTableModel1.setRowCount(0);
            for (ReceiptDTO receiptDTOEntity : receiptDTOTransactionList) {
                try {
                    transactionTableModel1.setRowCount(transactionTableModel1.getRowCount() + 1);
                    transactionTableModel1.setValueAt(receiptDTOEntity.getReceiptNo(), transactionTableModel1.getRowCount() - 1, 0);
                    java.util.Date date = (java.util.Date) Constants.simpleDateFormatDatabaseWithDash.parse(receiptDTOEntity.getDate().trim());
                    transactionTableModel1.setValueAt(Constants.DATE_FORMATER.format(date), transactionTableModel1.getRowCount() - 1, 1);
                    transactionTableModel1.setValueAt(receiptDTOEntity.getAccountName(), transactionTableModel1.getRowCount() - 1, 2);
    ...
    public class MessageBox {
        ...
        String filechoose() {
            ...
            int r = chooser.showSaveDialog(ReceiptForm.this);
...
//                    String xmlCode = TagsHelper1.exportDayBookTally(IDMapSet);
    ...
    private void initTransactionList() throws SQLException, Exception {
        receiptDTOTransactionList = ReceiptDAO.getTransactionList();
        bindDTOToTransactionTable(receiptDTOTransactionList);
    }
}
